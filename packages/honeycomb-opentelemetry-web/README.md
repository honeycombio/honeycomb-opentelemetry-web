...
| logExporters                      | optional                                         | LogRecordExporter[]               |                         | Array of [log record exporters](https://opentelemetry.io/docs/languages/js/exporters)                                                                                                             |
| logRecordProcessors               | optional                                         | LogRecordProcessor[]              |                         | Array of log record processors to apply to all emitted log records. Applied in addition to the default `BatchLogRecordProcessor` that wraps each configured log exporter.                         |
...
| shouldIgnoreError | optional | function | n/a | A callback function receiving `(error, event)` that returns `true` when no span should be recorded for that error, e.g. to drop errors coming from browser extensions. |
